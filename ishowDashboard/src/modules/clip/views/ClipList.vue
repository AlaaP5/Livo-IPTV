<script setup lang="ts">
import { reactive, ref } from "vue";
import type { DataTableHeader } from "../../../types/data-table-header.types";
import { createBooleanOptions } from "../../../utils/status-options";
import type {
  ILookupRequest,
  ILookupResponse,
} from "../../../types/lookup.types";
import lookupService from "../../../services/lookup";
import { accessTypes, badges, transcodes } from "../../../types/metadata";
import { useClipStore } from "../stores/clip";
import type {
  IFilterClipResponse,
} from "../types/clip-response";
import type { IFilterClipRequest } from "../types/filter-request";
import clipService from "../services/clipService";
import CreateClip from "./CreateClip.vue";
import UpdateClip from "./UpdateClip.vue";
import { getTranscodeStatusColor } from "../../../utils/transcodeStatusUtils";
import PublishClip from "./PublishClip.vue";
import streamService from "../../stream/services/streamService";

const clipStore = useClipStore();

const isValid = ref(true);

const totalItems = ref(0);

const accounts = ref<ILookupResponse[]>([]);

const badgesOptions = badges;

const transCodeOptions = transcodes;

const accessTypeOptions = accessTypes;

const items = ref<IFilterClipResponse[]>([]);

const posterUrls = reactive<Record<string, string>>({});

const lookupRequest = reactive<ILookupRequest>({
  name: "",
});

const credentials = reactive<IFilterClipRequest>({
  page: 1,
  size: 10,
  name: null,
  status: null,
  accountId: null,
  isActive: "1",
  isPublish: null,
  isKids: null,
  isSport: null,
  accessType: null,
  badge: null,
});

const headers: DataTableHeader[] = [
  { title: "Clip ID", key: "clipId" },
  { title: "Title En", key: "titleEn", align: "center" },
  { title: "Title Ar", key: "titleAr", align: "center" },
  { title: "Badge", key: "badge", align: "center" },
  { title: "Tags", key: "tags", align: "center" },
  { title: "Access Type", key: "accessType", align: "center" },
  { title: "Duration", key: "duration", align: "center" },

  { title: "Transcode Status", key: "transcodeStatus", align: "center" },
  { title: "Poster", key: "poster", align: "center" },
  { title: "Has Right", key: "hasRight", align: "center" },
  { title: "Is Kids", key: "isKids", align: "center" },
  { title: "Status", key: "active", align: "center" },
  { title: "Is Sports", key: "isSports", align: "center" },
  { title: "Is Publish", key: "isPublish", align: "center" },
  { title: "Actions", key: "actions", align: "center" },
];

const cases = createBooleanOptions("Active", "Not Active");

const publishCases = createBooleanOptions("Publish", "Not Publish");

const kidsCases = createBooleanOptions("Kids", "Not Kids");

const sportCases = createBooleanOptions("Sport", "Not Sport");

const createDialog = ref(false);
const publishDialog = ref(false);
const editDialog = ref(false);
const selectedClip = ref<IFilterClipResponse | null>(null);
const selectedClipId = ref<string>("");

const openEditDialog = (item: IFilterClipResponse) => {
  selectedClipId.value = item.clipId;
  editDialog.value = true;
};

const openPublishDialog = (item: IFilterClipResponse) => {
  selectedClip.value = null;
  setTimeout(() => {
    selectedClip.value = item;
    publishDialog.value = true;
  });
};

const onCreated = () => {
  handleFilters();
};

const onPublished = () => {
  handleFilters();
};

const onUpdated = () => {
  handleFilters();
};

async function getAccounts() {
  accounts.value = await lookupService.getAllAccounts(lookupRequest);
}

async function handleFilters() {
  const response = await clipService.filterClips(credentials);

  items.value = response.data ?? [];
  totalItems.value = response.totalCount;

  await Promise.all(
    items.value.map(async (item) => {
      if (item.poster?.generatedPath) {
        try {
          const imageUrl = await streamService.fetchImage(
            item.poster.generatedPath,
          );
          posterUrls[item.clipId] = imageUrl;
        } catch (error) {
          console.error("Failed to load poster for clip", item.clipId, error);
        }
      }
    }),
  );
}

const updatePage = (page: number) => {
  credentials.page = page;
  handleFilters();
};

const updatePageSize = (itemsPerPage: number) => {
  credentials.size = itemsPerPage;
  handleFilters();
};
</script>

<template>
  <v-container>
    <v-row justify="center">
      <v-col cols="12" style="max-width: 900px">
        <div class="title">Clip Service</div>
      </v-col>
    </v-row>
  </v-container>

  <v-tooltip text="Create Clip">
    <template #activator="{ props }">
      <v-btn
        v-bind="props"
        icon
        size="large"
        color="secondary"
        class="fab-btn"
        elevation="8"
        aria-label="Create Clip"
        @click="createDialog = true"
      >
        <v-icon>mdi-plus</v-icon>
      </v-btn>
    </template>
  </v-tooltip>

  <CreateClip v-model="createDialog" @created="onCreated" />

  <UpdateClip
    v-model="editDialog"
    :clip-id="selectedClipId"
    @updated="onUpdated"
  />

  <PublishClip
    :key="selectedClip?.clipId"
    v-model="publishDialog"
    :clip-item="selectedClip"
    @published="onPublished"
  />

  <v-row justify="center">
    <v-col cols="12" style="max-width: 1600px">
      <v-form ref="form" v-model="isValid">
        <v-card-text>
          <v-row>
            <v-col cols="14" md="3">
              <v-text-field
                style="height: 85px"
                v-model="credentials.name"
                label="Clip Title"
                variant="outlined"
                clearable
              ></v-text-field>
            </v-col>
            <v-col cols="14" sm="3">
              <v-select
                v-model="credentials.accountId"
                :items="accounts"
                item-title="name"
                item-value="id"
                label="Account"
                variant="outlined"
                clearable
                outlined
                dense
              >
                <template v-slot:prepend-item>
                  <v-text-field
                    v-model="lookupRequest.name"
                    @input="getAccounts"
                    label="Search Accounts"
                    dense
                    clearable
                    outlined
                    class="mx-2 mt-2"
                    dir="rtl"
                  ></v-text-field> </template
              ></v-select>
            </v-col>

            <v-col cols="12" md="2">
              <v-select
                v-model="credentials.status"
                label="Trans Code Status"
                :items="transCodeOptions"
                item-value="value"
                item-title="title"
                variant="outlined"
                outlined
                clearable
              ></v-select>
            </v-col>

            <v-col cols="12" md="2">
              <v-select
                v-model="credentials.badge"
                label="Badge"
                :items="badgesOptions"
                item-value="value"
                item-title="title"
                variant="outlined"
                outlined
                clearable
              ></v-select>
            </v-col>

            <v-col cols="12" md="2">
              <v-select
                v-model="credentials.accessType"
                label="Access Type"
                :items="accessTypeOptions"
                item-value="value"
                item-title="title"
                variant="outlined"
                outlined
                clearable
              ></v-select>
            </v-col>

            <v-col cols="14" md="2">
              <v-select
                v-model="credentials.isActive"
                label="Clip Status"
                :items="cases"
                item-value="value"
                item-title="title"
                variant="outlined"
                outlined
                clearable
              ></v-select>
            </v-col>

            <v-col cols="14" md="2">
              <v-select
                v-model="credentials.isPublish"
                label="Publish Status"
                :items="publishCases"
                item-value="value"
                item-title="title"
                variant="outlined"
                outlined
                clearable
              ></v-select>
            </v-col>

            <v-col cols="14" md="2">
              <v-select
                v-model="credentials.isKids"
                label="Kids Status"
                :items="kidsCases"
                item-value="value"
                item-title="title"
                variant="outlined"
                outlined
                clearable
              ></v-select>
            </v-col>

            <v-col cols="14" md="2">
              <v-select
                v-model="credentials.isSport"
                label="Sport Status"
                :items="sportCases"
                item-value="value"
                item-title="title"
                variant="outlined"
                outlined
                clearable
              ></v-select>
            </v-col>

            <v-btn
              class="btn"
              :disabled="!isValid"
              color="primary"
              @click="onCreated"
            >
              Search
            </v-btn>
          </v-row>

          <v-row>
            <v-col cols="12">
              <AppTable
                :headers="headers"
                :items="items"
                :total-items="totalItems"
                :page="credentials.page"
                :items-per-page="credentials.size"
                :loading="clipStore.loading"
                height="500"
                @update:page="updatePage"
                @update:items-per-page="updatePageSize"
              >
                <template #actions="{ item }">
                  <v-btn icon @click="openEditDialog(item)">
                    <v-icon color="secondary">mdi-pencil</v-icon>
                  </v-btn>
                  <v-btn
                    :disabled="item.isPublish"
                    icon
                    @click="openPublishDialog(item)"
                  >
                    <v-icon>mdi-publish</v-icon>
                  </v-btn>
                </template>

                <template v-slot:item.tags="{ item }">
                  <div>
                    <v-chip
                      v-for="tag in item.tags"
                      :key="tag.id"
                      class="ma-1"
                      small
                    >
                      {{ tag.titleEn }}
                    </v-chip>
                  </div>
                </template>
                <template v-slot:item.poster="{ item }">
                  <img
                    v-if="posterUrls[item.clipId]"
                    :src="posterUrls[item.clipId]"
                    width="100"
                    height="60"
                    style="object-fit: cover; border-radius: 8px"
                  />
                  <v-progress-circular
                    v-else-if="clipStore.loading"
                    indeterminate
                    size="30"
                    width="2"
                    color="primary"
                  />
                  <span v-else>No Poster</span>
                </template>
                <template v-slot:item.transcodeStatus="{ item }">
                  <v-chip
                    :color="getTranscodeStatusColor(item.transcodeStatus)"
                    small
                    class="ma-1"
                  >
                    {{ item.transcodeStatus }}
                  </v-chip>
                </template>

                <template v-slot:item.actors="{ item }">
                  <div>
                    <v-chip
                      v-for="actor in item.actors"
                      :key="actor.id"
                      class="ma-1"
                      small
                    >
                      {{ actor.titleEn }}
                    </v-chip>
                  </div>
                </template>

                <template v-slot:item.active="{ item }">
                  <v-chip
                    :color="item.active ? 'green' : 'red'"
                    small
                    class="ma-1"
                  >
                    {{ item.active ? "Active" : "Not Active" }}
                  </v-chip>
                </template>

                <template v-slot:item.isKids="{ item }">
                  <v-chip
                    :color="item.isKids ? 'green' : 'red'"
                    small
                    class="ma-1"
                  >
                    {{ item.isKids ? "Yes" : "No" }}
                  </v-chip>
                </template>

                <template v-slot:item.isSports="{ item }">
                  <v-chip
                    :color="item.isSports ? 'green' : 'red'"
                    small
                    class="ma-1"
                  >
                    {{ item.isSports ? "Yes" : "No" }}
                  </v-chip>
                </template>

                <template v-slot:item.isPublish="{ item }">
                  <v-chip
                    :color="item.isPublish ? 'green' : 'red'"
                    small
                    class="ma-1"
                  >
                    {{ item.isPublish ? "Yes" : "No" }}
                  </v-chip>
                </template>

                <template v-slot:item.hasRight="{ item }">
                  <v-chip
                    :color="item.hasRight ? 'green' : 'red'"
                    small
                    class="ma-1"
                  >
                    {{ item.hasRight ? "Yes" : "No" }}
                  </v-chip>
                </template>
              </AppTable>
            </v-col>
          </v-row>
        </v-card-text>
      </v-form>
    </v-col>
  </v-row>
</template>

<style scoped>
.fab-btn {
  position: fixed;
  bottom: 40px;
  right: 40px;
  z-index: 1000;
  border-radius: 50%;
}
</style>
