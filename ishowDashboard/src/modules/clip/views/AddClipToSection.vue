<script setup lang="ts">
import { reactive, ref, watch } from "vue";
import type {
  ILookupRequest,
  ILookupResponse,
} from "../../../types/lookup.types";
import lookupService from "../../../services/lookup";
import { useClipStore } from "../stores/clip";
import type { IAddClipToSectionRequest } from "../types/clip-request";
import clipService from "../services/clipService";

interface Props {
  modelValue: boolean;
  sectionId: number | null;
}

const clipStore = useClipStore();

const isValid = ref(false);

const formRef = ref();

const topOptions = [
  { value: "1", title: "Top" },
  { value: "0", title: "Not Top" },
];

const lookupClipRequest = reactive<ILookupRequest>({
  name: "",
});

const clips = ref<ILookupResponse[]>([]);

const props = defineProps<Props>();
const emit = defineEmits<{
  (e: "update:modelValue", value: boolean): void;
}>();

const credentials = reactive<IAddClipToSectionRequest>({
  sectionId: null,
  isTop: null,
  clipId: "",
});

const resetCredentials = () => {
  credentials.sectionId = props.sectionId;
  credentials.isTop = null;
  credentials.clipId = "";
  lookupClipRequest.name = "";
};

watch(
  () => props.modelValue,
  (isOpen) => {
    if (isOpen) {
      resetCredentials();
      getClips();
    }
  }
);

async function getClips() {
  clips.value = await lookupService.getAllClips(lookupClipRequest);
}

async function confirm() {

  await clipService.addClipToSection(credentials);

  emit("update:modelValue", false);
  resetCredentials();
}
</script>

<template>
  <v-dialog :model-value="modelValue" max-width="800">
    <v-card prepend-icon="mdi-publish" title="Add Clip To Section">
      <v-divider></v-divider>
      <v-form ref="formRef" v-model="isValid">
        <v-card-text>
          <v-row>
            <v-col cols="14" sm="4">
              <v-select
                v-model="credentials.clipId"
                :items="clips"
                item-title="name"
                item-value="id"
                label="clip"
                variant="outlined"
                clearable
                outlined
                dense
              >
                <template v-slot:prepend-item>
                  <v-text-field
                    v-model="lookupClipRequest.name"
                    @input="getClips"
                    label="Search Clips"
                    dense
                    clearable
                    outlined
                    class="mx-2 mt-2"
                  ></v-text-field> </template
              ></v-select>
            </v-col>

            <v-col cols="12" md="4">
              <v-select
                v-model="credentials.isTop"
                label="Is Top"
                :items="topOptions"
                item-value="value"
                item-title="title"
                variant="outlined"
                outlined
                clearable
              ></v-select>
            </v-col>
          </v-row>
        </v-card-text>
      </v-form>
      <v-divider />

      <v-card-actions class="justify-end">
        <v-btn variant="text" @click="emit('update:modelValue', false)">
          Cancel
        </v-btn>
        <v-btn color="secondary" :loading="clipStore.loading" @click="confirm">
          add
        </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
