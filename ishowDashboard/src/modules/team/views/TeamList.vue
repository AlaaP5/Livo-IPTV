...
import ActivateTeam from "./ActivateTeam.vue";
import BulkUploadTeams from "./BulkUploadTeams.vue";
...
const items = ref<ITeamResponse[]>([]);

const logoUrls = reactive<Record<number, string>>({});

const credentials = reactive<IFilterTeamRequest>({
  page: 1,
  size: 10,
  active: "",
  search: "",
...
const selectedTeam = ref<ITeamResponse | null>(null);

const openEditDialog = (item: ITeamResponse) => {
  selectedTeam.value = item;
  editDialog.value = true;
};

const openActivateDialog = (item: ITeamResponse) => {
...
async function handleFilters() {
  const response = await teamService.filterTeams(credentials);
    ...
    items.value.map(async (item) => {
      if (item.logoPath) {
        try {
          ...
          console.error("Failed to load logo for team", item.id, error);
        }
      }
    }),
  );
...
<template>
  ...
  <CreateTeam v-model="createDialog" @created="onCreated" />

  <UpdateTeam
    v-model="editDialog"
    :team-item="selectedTeam"
    @updated="onUpdated"
  />
  ...
  <BulkUploadTeams v-model="bulkUploadDialog" @uploaded="onBulkUploadSuccess" />
