...
interface Props {
  modelValue: boolean;
  episodeItem: ISeasonEpisodesResponse | null;
}
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      ...
      form.rate = newVal.rate?.toString() || "";
...
watch(selectedVideoFiles, async (files) => {
...
async function submit() {
  ...
  await seriesService.updateEpisode(form);
