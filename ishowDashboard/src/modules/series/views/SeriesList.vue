...
import type { IFilterSeriesRequest } from "../types/filter-request";
import seriesService from "../services/seriesService";
import CreateSeries from "./CreateSeries.vue";
import UpdateSeries from "./UpdateSeries.vue";
import PublishSeries from "./PublishSeries.vue";
...
const items = ref<ISeriesResponse[]>([]);
...
const credentials = reactive<IFilterSeriesRequest>({
  page: 1,
  size: 10,
  name: null,
  accountId: null,
...
const headers: DataTableHeader[] = [
  ...
  { title: "Rating", key: "rate", align: "center" },
...
const selectedSeries = ref<ISeriesResponse | null>(null);
const selectedSeriesId = ref<string>("");

const openEditDialog = (item: ISeriesResponse) => {
  selectedSeriesId.value = item.seriesId;
  editDialog.value = true;
};

const openPublishDialog = (item: ISeriesResponse) => {
...
const goToSeriesSeasons = (seriesId: string) => {
  router.push({
...
async function handleFilters() {
  const response = await seriesService.filterSeries(credentials);
    ...
    items.value.map(async (item) => {
      if (item.poster?.generatedPath) {
        try {
          ...
          posterUrls[item.seriesId] = imageUrl;
        } catch (error) {
          console.error("Failed to load poster for series", item.seriesId, error);
        }
      }
    }),
  );
...
<template>
  <v-container>
    <v-row justify="center">
      <v-col cols="12" style="max-width: 900px">
        <div class="title">Series Service</div>
      </v-col>
    </v-row>
  </v-container>

  <v-tooltip text="Create Series">
  ...
  <CreateSeries v-model="createDialog" @created="onCreated" />
            ...
            <v-col cols="12">
              ...
              >
                <template #actions="{ item }">
                  ...
                  <v-btn icon @click="goToSeriesSeasons(item.seriesId)">
                ...
                <template v-slot:item.poster="{ item }">
                  <img
                    v-if="posterUrls[item.seriesId]"
                    :src="posterUrls[item.seriesId]"
                    width="100"
                    height="60"
