...
import type { IClipSectionRequest } from "../../clip/types/clip-request";
import AddClipToSection from "./AddClipToSection.vue";
import RemoveSectionClip from "./RemoveSectionClip.vue";
import ChangeStatusClip from "./ChangeStatusClip.vue";
...
const items = ref<ISectionClipResponse[]>([]);
const selectedClip = ref<ISectionClipResponse | null>(null);
...
const credentials = reactive<IClipSectionRequest>({
  sectionId: null,
  isTop: null,
  page: 1,
  size: 10,
...
const headers: DataTableHeader[] = [
  { title: "Clip ID", key: "clipId", align: "center" },
...
const openRemoveDialog = (item: ISectionClipResponse) => {
  selectedClip.value = null;
  setTimeout(() => {
    selectedClip.value = item;
    removeDialog.value = true;
  });
};

const openChangeDialog = (item: ISectionClipResponse) => {
  selectedClip.value = null;
  setTimeout(() => {
    selectedClip.value = item;
...
const onChanged = (updatedItem: ISectionClipResponse) => {
  const index = items.value.findIndex(
    (item) =>
      item.clipId === updatedItem.clipId &&
...
async function handleFilters() {
  const response = await sportService.viewSectionClips(credentials);
    ...
    items.value.map(async (item) => {
      if (item.poster?.generatedPath) {
        try {
          ...
          logoUrls[item.clipId] = imageUrl;
        } catch (error) {
          console.error("Failed to load logo for channel", item.clipId, error);
        }
      }
    }),
  );
...
<template>
  <v-container>
    <v-row justify="center">
      <v-col cols="12" style="max-width: 900px">
        <div class="title">Clips For Section ({{ sectionName }})</div>
      </v-col>
    </v-row>
  </v-container>

  <v-tooltip text="add Clip To Section">
    <template #activator="{ props }">
      <v-btn
        ...
        class="fab-btn"
        elevation="8"
        aria-label="add Clip To Section"
  ...
  <RemoveSectionClip
    ...
    :section-clip-item="selectedClip"
  />
            ...
            <v-col cols="12">
              ...
              >
                ...
                <template v-slot:item.poster="{ item }">
                  <img
                    v-if="logoUrls[item.clipId]"
                    :src="logoUrls[item.clipId]"
                    width="80"
                    height="80"
