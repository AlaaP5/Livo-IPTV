...
import type { IAddTvToSectionRequest } from "../types/tv-request";
...
const credentials = reactive<IAddTvToSectionRequest>({
  sectionId: null,
  isTop: null,
  tvProgramId: "",
});
...
const resetCredentials = () => {
  ...
  credentials.tvProgramId = "";
  lookupTvRequest.name = "";
};
...
async function getTvs() {
  tvPrograms.value = await lookupService.getAllTvPrograms(lookupTvRequest);
}

async function confirm() {

  await tvProgramService.addTvToSection(credentials);
...
<template>
  <v-dialog :model-value="modelValue" max-width="800">
    <v-card prepend-icon="mdi-publish" title="Add Tv Program To Section">
      <v-divider></v-divider>
      <v-form ref="formRef" v-model="isValid">
        <v-card-text>
          <v-row>
            <v-col cols="14" sm="4">
              <v-select
                v-model="credentials.tvProgramId"
                :items="tvPrograms"
                item-title="name"
                item-value="id"
                label="Tv Program"
