package com.rand.ishowApi.kids.mobile.service;
...
import com.rand.ishowApi.kids.mobile.mapper.KidsMobileMapper;
...
public class KidsMobileService {

    private final OpenSearchClient openSearchClient;
    private final KidsMobileMapper kidsMobileMapper;

    public KidsMobileResponse getKidsMobile() throws IOException {
        ...
        MsearchResponse<JsonNode> response = openSearchClient.msearch(ms -> ms
                        .searches(Arrays.asList(
                                // KIDS BANNER
                                RequestItem.of(r -> r
                                        .header(h -> h.index(KidsIndex.KIDS.getIndexName()))
                                        .body(b -> b
                                ...
                                // MOVIES
                                RequestItem.of(r -> r
                                        .header(h -> h.index(MoviesIndex.MOVIES_KIDS.getIndexName()))
                                        .body(b -> b
                                ...
                                // SERIES
                                RequestItem.of(r -> r
                                        .header(h -> h.index(SeriesIndex.SERIES_KIDS.getIndexName()))
                                        .body(b -> b
                                                .query(q -> q.bool(buildCommonFilter(rightsRequired)))
                                        )
                                ),
                                ...
                                // TV PROGRAM
                                RequestItem.of(r -> r
                                        .header(h -> h.index(TvProgramIndex.TV_PROGRAM_KIDS.getIndexName()))
                                        .body(b -> b
                                                .query(q -> q.bool(buildCommonFilter(rightsRequired)))
                                        )
                                ),
                                ...
                                // CLIPS
                                RequestItem.of(r -> r
                                        .header(h -> h.index(ClipsIndex.CLIPS_KIDS.getIndexName()))
                                        .body(b -> b
                                                .query(q -> q.bool(buildCommonFilter(rightsRequired)))
                                        )
                                ),
                                ...
                                // CHANNEL
                                RequestItem.of(r -> r
                                        .header(h -> h.index(ChannelIndex.CHANNEL_KIDS.getIndexName()))
                                        .body(b -> b
                                                .query(q -> q.bool(buildCommonFilter(rightsRequired)))
                                        )
                                )
        ...
        return kidsMobileMapper.mapToKidsMobileResponse(response, lang);
    }
